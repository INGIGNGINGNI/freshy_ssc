<?php 
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = $conn->prepare("UPDATE champion SET firstname = :firstname, lastname = :lastname, nickname = :nickname, fos = :fos, position = :position, img = :img WHERE id = :id");
        $sql->bindParam(":id", $id);
...
<body>
    <div class="container mt-5">
        <h1>Edit Data</h1>
        <hr>
        <form action="editchampion_db.php" method="post" enctype="multipart/form-data">
            <?php
                if (isset($_GET['id'])) {
                        $id = $_GET['id'];
                        $stmt = $conn->query("SELECT * FROM champion WHERE id = $id");
                        $stmt->execute();
                        $data = $stmt->fetch();
                }
            ?>
