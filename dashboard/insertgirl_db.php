        ...
        if (in_array($fileActExt, $allow)) {
            if ($img['size'] > 0 && $img['error'] == 0) {
                if (move_uploaded_file($img['tmp_name'], $filePath)) {
                    $sql = $conn->prepare("INSERT INTO freshmen_girl(firstname, lastname, nickname, fos, description, position, img) 
                    ...
                    if ($sql) {
                        ...
                        header("refresh:2; url=freshmengirl.php");
                    } else {
                        $_SESSION['error'] = "Data has not been inserted successfully";
                        header("location: freshmengirl.php");
                    }
                }
            }
        }
